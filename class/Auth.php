...
class Auth {
    ...
    public function register($db, $username, $password, $email){
        ...
        $token = str::random(60);
        $db->query("INSERT INTO users SET username = ?, password = ?, email = ?, confirmation_token = ?", [$username,
 $password, $email, $token ]);
        $user_id = $db->lastInsertId();
        mail($email, 'Confirmation de votre compte', "Afin de valider votre compte, merci de cliquer sur <a href=\"http://cartomagie.dev/confirm.php?id=$user_id&token=$token\" target='_blank'>ce lien</a>");
    }
    ...
    public function connectFromCookie($db){
        if (isset($_COOKIE['remember']) && !$this->user()){
            ...
            $user = $db->query('SELECT * FROM users WHERE id = ?', [$user_id])->fetch();
            if ($user){
                $expected = $user_id. '=='. $user->remember_token . sha1($user_id . 'cartomagie');
    ...
    public function remember($db, $user_id){
        ...
        setcookie('remember', $user_id. '=='. $remember_token . sha1($user_id . 'cartomagie'), time() + 60 * 60 * 24 * 7);
    }
    ...
    public function resetPassword($db, $email){
        ...
        if($user){
            ...
            mail($_POST['email'], "<meta charset='UTF-8'/>Réinitialisation de votre mot de passe", "<meta charset='UTF-8'/> Afin de réinitialiser votre mot de passe, merci de cliquer sur <a href=\"http://cartomagie.dev/reset.php?id={$user->id}&token=$reset_token\" target='_blank'>ce lien</a>");
            return $user;
        }
        return false;
    }
